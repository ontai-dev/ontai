...
**Purpose:** Compiler enable bundle phasing (Gap 29) — restructure the enable bundle into sequenced application phases with Guardian-first ordering. The enable bundle currently produces crds.yaml + operators.yaml + rbac.yaml + leaderelection.yaml + rbacprofiles.yaml as flat output. Phase sequencing must ensure: (1) Guardian CRDs and operator land first, (2) Guardian's seam-system namespace label (seam.ontai.dev/webhook-mode=exempt) is stamped before other operators are applied, (3) remaining operators follow after guardian reaches a running state. This aligns with the WS3 bootstrap label contract added this session.

**Pre-conditions for next session:**
- guardian 25c9e93 pushed to remote (done — this session)
- WS3 bootstrap label contract (CheckBootstrapLabels) implemented in guardian — compiler enable must stamp the exempt label on seam-system before starting guardian
- conductor-schema.md §compiler and session/32 WS3 are the primary references for enable bundle structure

**Also queued (Governor to schedule):**
- Domain-core scaffold: Schema Engineer session to define DomainLineageIndex CRD at ONTAI Domain Core layer
