...
**Role:** Governor
**Purpose:** Schedule platform update to populate the three new RunnerConfig self-operation fields (MaintenanceTargetNodes, OperatorLeaderNode, SelfOperation) on day-2 CR creation. All 8 day-2 reconcilers currently submit Jobs without these fields — they must be wired at RunnerConfig creation time using the Kubernetes downward API for leader pod node resolution.

**RunnerConfig schema change (conductor da9e871):** Three new first-class scheduling fields added to RunnerConfigSpec per conductor-schema.md §13:
- `MaintenanceTargetNodes []string` — nodes targeted by the operation
- `OperatorLeaderNode string` — leader pod's node (downward API at creation time)
