...
| ID | Component | Description |
|----|-----------|-------------|
| COMPILER-BL-PERMISSIONSET-DEFECT | conductor (compiler) | writeBootstrapPermissionSets still generates per-operator PermissionSets (guardian-permissions, wrapper-permissions, platform-permissions, seam-core-permissions, conductor-permissions). buildOperatorRBACProfile still emits permissionSetRef: {op.Name}-permissions instead of management-maximum. Violates CS-INV-008 and the locked three-layer architecture (guardian-schema.md §19). Also blocks Step 5 of import-mode onboarding: conductor-permissions lacks the infrastructuretalosclusters/status update rule required for management conductor to write InfrastructureTalosCluster.status.conductorHandshake (guardian-schema.md §20 Step 5 implementation gap). Fix: (1) add infrastructuretalosclusters/status get/update/patch to conductor-permissions; (2) keep only management-maximum in writeBootstrapPermissionSets; (3) update buildOperatorRBACProfile to emit permissionSetRef: management-maximum. Regenerate both enable bundles after fix. Documented in guardian-schema.md §19 and §20 amendment log 2026-04-26. |
