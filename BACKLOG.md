...
|----|-----------|--------|------------|
| K8S-DRIFT-CONFIRM | platform, conductor | 2026-05-06 | Validated via synthetic injection. Patched `spec.kubernetesVersion=1.32.2` on tenant-local `InfrastructureTalosCluster ccs-dev` (ont-system). Conductor k8s version drift loop detected mismatch vs observed=1.32.3 within ~15s; created `drift-k8s-version-ccs-dev` DriftSignal in `seam-tenant-ccs-dev`. `DriftSignalReconciler` advanced signal to `queued`. Stale pre-session/25 UpgradePolicy and DriftSignal deleted. Note: `DriftSignalReconciler.handleTalosVersionDrift` routes k8s drift signals but skips them (non-parseable Talos version format) -- automatic corrective UpgradePolicy for k8s drift is not yet implemented; detection loop confirmed working. Reverted spec to 1.32.3 and deleted synthetic signal. |
...
| PLATFORM-BL-MACHINECONFIG-RESTORE | platform, conductor | 2026-05-06 | `machineconfig-restore` Conductor capability + `TalosMachineConfigRestore` CRD + `MachineConfigRestoreReconciler` implemented. Downloads per-node config from S3, applies via `ApplyConfiguration`, waits stable. Non-fatal per node. 5 unit tests pass. `TalosMachineConfigBackupSchedule` + `TalosEtcdBackupSchedule` schedule CRDs and reconcilers added. All 3 reconcilers registered in main. Platform #26 + conductor #41 open, CI running. |
