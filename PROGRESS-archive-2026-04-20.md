Session/4-webhook-hardening-and-compiler-fixes opened 2026-04-20. Role: Governor. Repos: guardian, conductor, platform, ontai root. WS1: branches created in guardian, conductor, ontai root (platform branch created during WS7). WS2: Governor directive "Context Compaction Safety Protocol" (5 rules) appended to CLAUDE.md as Section 16; force-added and committed (9fcb543). WS3: Governor directive "e2e CI Contract and Skip-Reason Standard" (6 rules) appended to CLAUDE.md as Section 17; committed (153f31a). WS4 (G-BL-CR-IMMUTABILITY): operator-authorship guard implemented in guardian -- operator_cr_guard.go (pure decision, EvaluateOperatorAuthorship), operator_cr_handler.go (handler), server.go RegisterOperatorCRGuard, config/webhook/operator-cr-validating-webhook-configuration.yaml, 10 unit tests all pass, 6 e2e stubs (guardian commit 16c85f4). WS5 (C-COREDNS-PATCH): removed INV-001-violating writeCoreDNSDSNSPatchScript from compile_enable.go; updated phase 05 meta to reference C-COREDNS-PATCH and CI step 7a; added TestEnable_Phase05_NoDSNSPatchScript + TestEnable_Phase05_MetaReferencesCI; updated determinism and output file list tests; 3 e2e stubs (conductor commit a2eada4). WS6 (C-KUEUE-WEBHOOK): moved Kueue webhook scoping from Step 7d to Phase 00 in enable-ccs-mgmt.sh (immediately after kueue-controller.yaml apply, with wait_crd guard); added TestEnable_Phase00_KueueWebhookScopingDocumented; 3 e2e stubs (conductor commit a0a4c53). WS7 (C-34): changed platform API TalosClusterSpec.CAPI from CAPIConfig to *CAPIConfig so omitempty suppresses the block when nil (management cluster path); added CAPIEnabled() helper; updated deepcopy, 5 controllers, 3 test files in platform (platform commit 7f70533); updated buildTalosCluster + 2 management cluster builders in conductor compile.go; added TestBootstrap_CAPIDisabled_NoCapiBlockInCR; updated TestBootstrap_TalosClusterHasCorrectSpec (conductor commit f7c66ad). Test counts: guardian 10 new unit tests + 6 e2e stubs; conductor all compiler tests pass; platform all unit tests pass. All 4 backlog items closed. 2 Governor directives codified. G-BL-CR-IMMUTABILITY CLOSED. C-COREDNS-PATCH CLOSED. C-KUEUE-WEBHOOK CLOSED. C-34 CLOSED.
Session/2-lineage-sync opened 2026-04-19. Role: Controller Engineer. Repos: seam-core, guardian. Backlog item: SEAM-CORE-BL-LINEAGE. Diagnostic findings: (1) All 9 GVKs in RootDeclarationGVKs are correctly registered with matching group/version strings -- no GVK mismatch found. PackInstance GVK (infra.ontai.dev/v1alpha1/PackInstance) confirmed correct against wrapper groupversion_info.go. Manager in main.go registers all 9 via loop. (2) Status().Patch() path in ensureLineageSyncedTrue is already uniform across all 9 GVKs -- no production code change required. (3) Root cause of PackInstance LineageSynced=False: test gap -- no test exercised the PackInstance reconcile path with WithStatusSubresource. Added TestLineageReconciler_PackInstanceCreatesILIAndSetsLineageSynced as permanent regression guard. (4) Root cause of PermissionSnapshot LineageSynced=False: permissionsnapshot_controller.go initialized LineageSynced=False on first observation, but PermissionSnapshot is a derived object not in the 9 root GVKs, so the InfrastructureLineageController never watches it and LineageSynced=False was never transitioned to True. Fix: removed the erroneous initialization block (Step 4, lines 98-110) from permissionsnapshot_controller.go. Updated docstring, renumbered steps. Updated permissionsnapshot_types.go comment. Replaced two wrong tests (LineageSyncedInitialized + LineageSyncedNotOverwrittenOnReconcile) with TestPermissionSnapshotReconciler_LineageSyncedAbsent as permanent guard. Tests before: seam-core all pass (16 tests), guardian unit all pass (integration/webhook FAIL pre-existing envtest missing). Tests after: seam-core all pass (17 tests, new PackInstance test added), guardian unit all pass. Build clean both repos. seam-core commit 52de8d3. guardian commit c36ffd3. SEAM-CORE-BL-LINEAGE CLOSED.
Session 22 opened. Role: Controller Engineer. Component: seam-core. Objective: WS1 — full InfrastructureLineageIndex spec, controller-gen wiring; WS2 — LineageController implementation. Session 22 closed. WS1: ILI spec complete (rootBinding, descendantRegistry, policyBindingStatus), controller-gen wired (Makefile, generate-deepcopy, generate-crd), manager entry point with 9 GVK registrations, leader election in seam-system. WS2: LineageReconciler watching all 9 root-declaration GVKs across 3 operators, governance.infrastructure.ontai.dev sub-prefix reserved, LineageSynced ownership transfer (False/LineageControllerAbsent → True/LineageIndexCreated), controller-authored annotation on ILI, deterministic ILI naming (lowercasekind-name), idempotency guard via annotation check + ILI re-creation path. 9 unit tests green. Key investigation: fake client Status().Patch() fails for unregistered foreign unstructured GVKs unless WithStatusSubresource() is called with the unstructured object carrying its GVK — fix applied in tests. Build clean. Commit be0aaa1. LineageController deferred item CLOSED. Remaining deferred: SealedCausalChain immutability webhook, LineageIndex authorship webhook, CapabilityRBACProvision executor, Wrapper SealedCausalChain embedding.
